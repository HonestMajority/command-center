...
impl Runtime for TmuxSessionRuntime {
    ...
    fn create_worktree(
        ...
        runtime::TmuxRuntime.create_worktree(repo_root, name, perms, branch, hooks_source)
    }

    fn recreate_worktree(&self, repo_root: &Path, work_dir: &Path) -> anyhow::Result<()> {
        runtime::TmuxRuntime.recreate_worktree(repo_root, work_dir)
    }
    ...
    fn setup_dir_config(
        ...
        runtime::TmuxRuntime.setup_dir_config(hooks_source, work_dir, perms)
    }
