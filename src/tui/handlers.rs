...
/// Shared: go to the selected task's environment (or reopen if closed).
async fn goto_task_env<R: Runtime>(state: &mut ScreenState, app: &ClatApp<R>) {
    if let Some(task) = state.selected_task() {
        if task.status.is_running() {
            ...
            match app.reopen(&id).await {
                Ok(session_name) => {
                    ...
                    app.goto_task_env(&session_name);
                }
                Err(e) => {
...
pub(super) async fn tick_refresh<R: Runtime>(
    ...
    let active = state.active_state_mut();
    active.task_list.update_active_envs(app.active_task_envs());
